...
class ComplianceReview(Document):
	def validate(self):
		# fetch targets from goal
		if not self.reviews:
			for d in frappe.get_doc('Compliance Goal', self.goal).objectives:
...
def review():
	...
	for goal in frappe.get_list("Compliance Goal", fields=['name', 'frequency', 'date', 'weekday']):
...
def create_review(goal):
	goal = frappe.get_doc("Compliance Goal", goal)
